...
colors = {0: (153, 153, 255), #Tooth
              1: (255, 128, 128), #Implant
              2: (164, 196, 0), #Metal
              3: (244, 114, 208), #Maxilla
              4: (229, 20, 0), #Mandible
              5: (250, 104, 0), #Inf alv nerve
              6: (0, 171, 169), #Maxillary sinus
              7: (27, 161, 226), #Plastic restoration
              8: (0, 80, 239), #Endodontic treatment
              9: (227, 200, 0), #Crown/pontic
              10: (203, 203, 203), #Metallic restoration
              11: (130, 90, 44), #Screw
              12: (162, 0, 37), #Supernumerary
    }
...
def drawPred(frame, classId, left, top, right, bottom):
    # Draw a bounding box.
    frame_tmp = frame
    
    cv2.rectangle(frame_tmp, (left, top), (right, bottom), colors[int(classId)], 3)
    ...
    label = '%s' % (obj_list[int(classId)]) #uncomment if you have a class_lists.txt with class names in it 

    #Display the label at the top of the bounding box
    labelSize, baseLine = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 1, 2)
    top = max(top, labelSize[1])
    frame_tmp = cv2.rectangle(frame_tmp, (left, int(top - round(1.5*labelSize[1]))), (left + int(round(1,75*labelSize[0])), top + baseLine), colors[int(classId)], 1)
    frame_tmp = cv2.putText(frame_tmp, label, (left, top), cv2.FONT_HERSHEY_SIMPLEX, 1, colors[int(classId)], 2)
    return frame_tmp

def drawGT(frame, classId, left, top, right, bottom):
    # Draw a bounding box.
    frame_gt = frame
    cv2.rectangle(frame_gt, (left, top), (right, bottom), colors[int(classId)], 3)
    ...
    top = max(top, labelSize[1])
    frame_gt = cv2.rectangle(frame_gt, (left, int(top - round(1.5*labelSize[1]))), (left + int(round(1.75*labelSize[0])), top + baseLine), colors[int(classId)], 1)
    frame_gt = cv2.putText(frame_gt, label, (left, top), cv2.FONT_HERSHEY_SIMPLEX, 1, colors[int(classId)], 2)
    return frame_gt
...
def main():
    ...
    for name in filenames:
        ...
        for line in content_det:
            ...
            image_det = drawPred(frame_orig_1, obj_id, left, top, right, bottom)
